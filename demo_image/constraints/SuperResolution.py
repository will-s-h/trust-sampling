...
import warnings
warnings.filterwarnings("ignore", message="Failed to load image Python extension") # filter torchvision warning
...
class SuperResolutionConstraint():
    ...
    def _format_image(self, ref_image):
        ...
        if isinstance(ref_image, torch.Tensor):
            ...
            if ref_image.shape[-3:] == (3, 64, 64):
                return ref_image  # no additional formatting needed
            elif ref_image.shape[-3:] == (3, 256, 256):
                if ref_image.dim() == 3:
                    return self.down_sample(ref_image.unsqueeze(0)).squeeze()
                else:
                    return self.down_sample(ref_image)
            else:
