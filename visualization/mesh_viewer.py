...
def new_uv_sphere(radius=1.0, count=None, transform=None):
    """
    ...
    return trimesh.creation.revolve(linestring=linestring,
                   sections=count[1],
                   transform=transform,
...
class MeshViewer(object):
    ...
    def add_forces_seq(self, forces_seq, color=[1.0, 0.0, 0.0], radius=0.05, render_static=None):
        '''
        - forces_seq : List of (K, 2, 3) numpy arrays of force vectors (start/end) to visualize as sequence.
        '''

        pyrender_skeleton_seq = []
        for pid, forces in enumerate(forces_seq):
            if pid % 200 == 0:
                print('Caching pyrender forces mesh %d/%d...' % (pid, len(forces_seq)))
            ...
            for i in range(forces.shape[0]):
                ...
                if np.linalg.norm(p1 - p2) < 1e-6:
                    segment = np.array([[-1.0, -1.0, -1.0], [-1.0001, -1.0001, -1.0001]])
                else:
                    segment = np.array([p1, p2])
                # print(segment)

                cyl_mesh = trimesh.creation.cylinder(radius, height=None, segment=segment)

                # TODO: tmp, viz first force as red
                if i == 0:
                    cyl_mesh.visual.vertex_colors = [0.8, 0.0, 0.0]
                else:
        ...
        if render_static is None:
            ...
            self.add_static_meshes(
                [pyrender_skeleton_seq[i] for i in range(len(pyrender_skeleton_seq)) if i % render_static == 0])

    def add_spheres_seq(self, ball_seq, color=[1.0, 0.0, 0.0], render_static=None):
        '''
            Add a sequence of spheres where each of them may have different radius
            - ball_seq : List of (K, 4) numpy arrays of sphere (centers, radius) to visualize as sequence.
        '''

        pyrender_spheres_seq = []

        for pid, balls in enumerate(ball_seq):
            if pid % 200 == 0:
                print('Caching pyrender spheres mesh %d/%d...' % (pid, len(ball_seq)))
            ...
            for i in range(balls.shape[0]):
                ...
                tf = np.eye(4)
                tf[:3, 3] = center.copy()
                sm = new_uv_sphere(radius=r, transform=tf)
                sm.visual.vertex_colors = color
                ball_mesh_list.append(sm)

            m = pyrender.Mesh.from_trimesh(ball_mesh_list)
            pyrender_spheres_seq.append(m)

        if render_static is None:
            self.add_pyrender_mesh_seq(pyrender_spheres_seq)
        else:
            self.add_static_meshes(
                [pyrender_spheres_seq[i] for i in range(len(pyrender_spheres_seq)) if i % render_static == 0]
            )
    ...
    def add_ground(
        self,
        ground_plane_seq=None, length=100.0, color0=[0.8, 0.9, 0.9], color1=[0.6, 0.7, 0.7],
        tile_width=0.5, xyz_orig=None, alpha=1.0,
        render_static=None
    ):
        '''
        If ground_plane is none just places at origin with +z up.
        If ground_plane is given [(a, b, c, d)] where a,b,c is the normal, d is offset,
        then this is rendered per step in the list. To more accurately place the floor
        ...
        for i in range(num_rows):
            for j in range(num_cols):
                ...
                vertices_nominal.append(cur_verts)
                faces.append(cur_faces)
                face_colors.append(cur_face_colors)

        vertices_nominal = np.concatenate(vertices_nominal, axis=0)
        faces = np.concatenate(faces, axis=0)
        face_colors = np.concatenate(face_colors, axis=0)

        if ground_plane_seq is None:
            ground_tri = trimesh.creation.Trimesh(vertices=vertices_nominal, faces=faces, face_colors=face_colors, process=False)
            ...
            pyrender_ground_plane_seq = []
            for pid, ground_plane in enumerate(ground_plane_seq):
                if pid % 200 == 0:
                    print('Caching pyrender connections mesh %d/%d...' % (pid, len(ground_plane_seq)))
                ...
                # transform floor
                vertices = np.dot(R, vertices_nominal.T).T + t.reshape((1, 3))

                ground_tri = trimesh.creation.Trimesh(vertices=vertices, faces=faces, face_colors=face_colors,
                                                      process=False)
                ground_mesh = pyrender.Mesh.from_trimesh(ground_tri, smooth=False)
                pyrender_ground_plane_seq.append(ground_mesh)

            if render_static is None:
                self.add_pyrender_mesh_seq(pyrender_ground_plane_seq)
            else:
                self.add_static_meshes(
                    [pyrender_ground_plane_seq[i] for i in range(len(pyrender_ground_plane_seq)) if i % render_static == 0])
        ...
        for seq_idx in range(len(self.animated_seqs)):
            ...
            if self.follow_camera and not self.use_intrins: # don't want to reset if we're going from camera view
                if self.animated_seqs_type[seq_idx] == 'mesh':
                    ...
                    if hasattr(self.viewer, '_trackball'):
                        self.viewer._trackball._n_pose = camera_pose.copy()
    ...
    def animate(self, fps=30):
        ...
        while self.use_offscreen or self.viewer.is_active:
            ...
            if not self.use_offscreen:
                cur_t_caption = {
                    "text": str(self.animation_frame_idx),
                ...
                self.viewer.viewer_flags['caption'] = [cur_t_caption]
